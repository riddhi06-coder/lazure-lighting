	   ...
	   <!--end sidebar wrapper-->
                        ...
                        <div class="row g-3">    
                            ...
                            <div class="tab-content" id="wizard-tabContent">
                                <div class="tab-pane fade show active" id="wizard-contact" role="tabpanel" aria-labelledby="wizard-contact-tab">
                                    ...
                                    <form class="row g-3 needs-validation custom-input" novalidate action="{{ route('manage-model-details.store') }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        ...
                                        <div class="col-md-6 mt-5">
                                            <label class="form-label" for="model_details_file">Upload Model Details (CSV/Excel) <span class="txt-danger">*</span></label>
                                            <input class="form-control" id="model_details_file" type="file" name="model_details_file" accept=".csv, application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required>
                                            <div class="invalid-feedback">Please upload a CSV or Excel file for Model Details.</div>
                                        </div>
                                    ...
                                    <!-- <form class="row g-3 mt-4" action="{{ route('upload.drawings_3d') }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <small class="text-secondary"><b>Note: Multiple ZIPs allowed. Max size per file 5MB.</b></small>
                                        </div>
                                    </form> -->
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const statusList = document.getElementById('uploadStatus');
                ...
                uploadBtn.addEventListener('click', function(e) {
                    ...
                    const subProductId = document.getElementById('hidden_sub_product_id').value;
                    if (!subProductId) return alert('Sub product ID is missing!');
                    ...
                    let currentFileGlobal = 0; // to track overall progress
                    ...
                    function uploadBatch() {
                        ...
                        function uploadNextInBatch() {
                            ...
                            console.log(`🚀 Uploading file ${currentFileGlobal + 1}/${files.length}:`, file.name);
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Prevent upload if no sub-product selected
                manualUploadBtn.addEventListener('click', function(e) {
                    if (!hiddenManualInput.value) {
                        e.preventDefault();
                        alert("Please select a Sub Product before uploading manuals.");
                    }
                });
            });
        </script>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const statusList = document.getElementById('manualUploadStatus');
                ...
                uploadBtn.addEventListener('click', function(e) {
                    ...
                    const subProductId = document.getElementById('hidden_sub_product_id_manual').value;
                    if (!subProductId) return alert('Sub product ID is missing!');
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const statusList = document.getElementById('drawings2DStatus');
                ...
                drawingsBtn.addEventListener('click', function(e) {
                    ...
                    function uploadBatch() {
                        ...
                        for (let i = start; i < end; i++) {
                            formData.append('drawings_2d[]', files[i]);
                        }
                        ...
                        .then(async res => {
                            ...
                            try {
                                ...
                                throw new Error("❌ Server did not return valid JSON. Check Laravel controller response.");
                            }
                        })
                        .then(data => {
                            if (data.success) {
                                ...
                                progressBar.style.width = percent + '%';
                                progressBar.textContent = `Batch ${currentBatch} of ${totalBatches} – ${percent}%`;
                                ...
                                console.error('❌ Upload failed:', data.message);
                                progressBar.textContent = `❌ Failed at batch ${currentBatch + 1}`;
                                alert('❌ Upload failed, check console.');
                                drawingsBtn.disabled = false;
                            }
                        })
                        .catch(err => {
                            console.error('❌ Upload error:', err);
                            progressBar.textContent = `❌ Error at batch ${currentBatch + 1}`;
                            alert('❌ Upload failed, check console.');
                            drawingsBtn.disabled = false;
                        });
                    }
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Update hidden input when main sub_product_id changes
                mainSelect.addEventListener('change', function() {
