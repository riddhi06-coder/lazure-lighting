                ...
                <div class="card">
                  <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="{{ route('manage-detailed-page.create') }}" class="btn btn-primary px-5 radius-30">+ Add Details</a>
                    </div>
                        ...
                        <table class="display" id="basic-1">
                            ...
                            <tbody>
                                @foreach($products as $key => $product)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $product->subProduct->sub_product ?? 'N/A' }}</td>
                                        <td>
                                            @if(!empty($product->thumbnail_image))
                                                <img src="{{ asset($product->thumbnail_image) }}" alt="Product Image" width="150" height="150">
                                            @else
                                                <span class="text-muted">No Image</span>
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('manage-detailed-page.edit', $product->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                            <form action="{{ route('manage-detailed-page.destroy', $product->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
